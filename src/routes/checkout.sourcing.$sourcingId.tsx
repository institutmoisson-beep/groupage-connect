...
export const Route = createFileRoute("/checkout/sourcing/$sourcingId")({
  head: () => ({
    meta: [
      { title: "Paiement sourcing — MSN Smart Payment" },
      { name: "robots", content: "noindex" },
    ],
  }),
  component: SourcingCheckoutPage,
});
...
function SourcingCheckoutPage() {
  ...
  const { data: order, isLoading } = useQuery({
    queryKey: ["sourcing-order", sourcingId],
    enabled: !!user,
    queryFn: async () => {
      const { data, error } = await supabase
        .from("custom_sourcing_orders")
        .select("id, product_name, final_total_xof, estimated_total_xof, status, quantity, user_id")
        .eq("id", sourcingId)
        .maybeSingle();
  ...
  const { data: existingProofs } = useQuery({
    queryKey: ["proofs-sourcing", sourcingId],
    enabled: !!user && !!sourcingId,
    queryFn: async () => {
  ...
  if (!user) {
    navigate({ to: "/auth", search: { redirect: `/checkout/sourcing/${sourcingId}` } as never });
    return null;
  }
  if (!order) {
    return (
  ...
  const paid = order.status === "paid" || order.status === "ordered_china" || order.status === "qc" || order.status === "shipped" || order.status === "transit" || order.status === "abidjan" || order.status === "delivered";
  ...
  async function submitProof() {
    ...
    if (!amount) return toast.error("Montant non fixé — attendez le devis MSN");
    setSubmitting(true);
    try {
      const { error } = await supabase.from("payment_proofs").insert({
        sourcing_order_id: sourcingId,
        user_id: user!.id,
      ...
      qc.invalidateQueries({ queryKey: ["proofs-sourcing", sourcingId] });
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-md flex-col bg-background">
      <header className="sticky top-0 z-40 flex items-center gap-2 border-b border-border bg-background/95 px-3 py-2.5 backdrop-blur">
        ...
        <h1 className="flex-1 text-sm font-semibold">MSN Smart Payment — Sourcing</h1>
      </header>
      ...
      <main className="flex-1 space-y-4 p-4 pb-24">
        <div className="rounded-2xl bg-gradient-brand p-4 text-primary-foreground shadow-brand">
          ...
          <div className="mt-1 font-display text-3xl font-black">{formatXOF(amount)}</div>
          <div className="mt-1 text-[11px] opacity-80 line-clamp-2">{order.product_name} · qté {order.quantity}</div>
        </div>
        ...
        {paid ? (
          <div className="rounded-xl border border-success/40 bg-success/10 p-4 text-center">
            ...
            <Link to="/sourcing" className="mt-3 inline-block text-xs font-semibold text-primary">
              Retour au sourcing →
            </Link>
          </div>
        ) : hasPendingProof ? (
          <div className="rounded-xl border border-secondary/40 bg-secondary/10 p-4 text-center">
            ...
            <p className="mt-1 text-xs text-muted-foreground">L'admin valide votre paiement sous peu.</p>
          </div>
        ) : (
          <>
            <section>
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-3 rounded-xl border p-3 text-left transition ${
                          active ? "border-primary bg-primary/5 shadow-brand" : "border-border bg-card hover:border-primary/40"
                        }`}
                      >
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-[11px] text-muted-foreground capitalize">{m.type.replace("_", " ")}</div>
                        </div>
            ...
            {selected && (
              <section className="space-y-3 rounded-xl border border-primary/30 bg-primary/5 p-3">
                <h3 className="text-xs font-bold uppercase tracking-wider text-primary">2. Envoyez le paiement</h3>
                {selected.account_identifier && <CopyRow label="Destinataire" value={selected.account_identifier} />}
                ...
                <CopyRow label="Montant exact" value={formatXOF(amount)} />
            ...
            {selected && (
              <section className="space-y-3">
                <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground">3. Envoyez la preuve</h3>
                <label className="block">
                  <span className="text-xs font-semibold text-muted-foreground">Référence / N° de transaction</span>
                  <input
                    value={reference}
                ...
                <div>
                  <span className="text-xs font-semibold text-muted-foreground">Capture d'écran de la transaction</span>
                  <input
                    ref={fileRef}
                    type="file"
                    accept="image/*"
